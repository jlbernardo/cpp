...
2.1) Create a vector of integers that will store the Jacobsthal sequence up to the size of the pend vector. So, if pend size is `8` and the sequence goes `[0, 1, 1, 3, 5, 11]`, you'll go up to `5`.
...
<br>
2.2) Create a vector of integers that will store our insertion order list. To create this one, we will add one Jacobsthal number, then look back at our list and see if, between this jacobsthal number and the last number added to the list, there is any other number missing. If there is, we will add the missing numbers in descending order. For example:
